...
def retry_with_policy(policy: RetryPolicy):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            # All attempts failed
            logger.error(f"All {policy.max_attempts} attempts failed for {func.__name__}")
...
async def async_retry_with_policy(policy: RetryPolicy):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            logger.error(f"All {policy.max_attempts} async attempts failed for {func.__name__}")
            raise last_exception
...
class ErrorHandler:
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_recent_errors': self.error_history[-10:] if self.error_history else []
        }
...
class HealthChecker:
        ...
        for name, check_func in self.checks.items():
            try:
                ...
                if result.get('status') == 'warning':
                    logger.warning(f"Health check warning ({name}): {result.get('message')}")
                elif result.get('status') == 'error':
                    logger.error(f"Health check error ({name}): {result.get('message')}")
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(1 for r in self.last_check_results.values() 
                           if r.get('status') == 'healthy')
        warning_count = sum(1 for r in self.last_check_results.values() 
                           if r.get('status') == 'warning')
        error_count = sum(1 for r in self.last_check_results.values() 
