...
class USGSClient:
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if attempt < self.max_retries:
                ...
                logger.warning(f"Request failed, retrying in {wait_time}s: {last_exception}")
    ...
    def get_events(
        ...
        if latitude is not None and longitude is not None:
            ...
            raise ValueError("Geographic search requires latitude, longitude, and optionally max_radius_km")
        ...
        async with self.throttler:
            ...
            try:
                async with self.session.get(url, params=params) as response:
                    ...
                    logger.info(f"Successfully fetched {len(data.get('features', []))} events")
                    return data
