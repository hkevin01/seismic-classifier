...
class DataValidator:
    ...
    def validate_usgs_response(self, data: Dict[str, Any]) -> bool:
        ...
        if valid_ratio < 0.8:  # 80% threshold
            raise DataQualityError(f"Only {valid_ratio:.1%} of events are valid")

        logger.info(
            f"USGS validation passed: {valid_events}/{len(features)} events valid"
        )
        return True
    ...
    def validate_waveform_stream(self, stream: Stream) -> bool:
        ...
        if valid_ratio < 0.7:  # 70% threshold
            raise DataQualityError(f"Only {valid_ratio:.1%} of traces are valid")

        logger.info(
            f"Stream validation passed: {valid_traces}/{len(stream)} traces valid"
        )
        return True
    ...
    def _validate_trace(self, trace: Trace) -> bool:
        ...
        if not (
            self.quality_thresholds["min_sampling_rate"]
            <= sr
    ...
    def check_data_gaps(self, stream: Stream) -> Dict[str, Any]:
        ...
        """
        gap_info = {"total_gaps": 0, "gap_ratio": 0.0, "gap_details": []}
        ...
        if total_expected_samples > 0:
            gap_info["gap_ratio"] = 1.0 - (
                total_actual_samples / total_expected_samples
            )

        logger.info(
            f"Gap analysis: {gap_info['total_gaps']} gaps, "
    ...
    def generate_validation_report(
        self, stream: Stream, output_path: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        ...
        report["quality_metrics"]["quality_score"] = self.calculate_data_quality_score(
            stream
        )
        ...
        if report["quality_metrics"]["gap_info"]["gap_ratio"] > 0.1:
            report["recommendations"].append(
                "Significant data gaps detected - consider interpolation"
            )
