...
class SeismicDatabase:
    ...
    def load_waveform_data(self, waveform_id: str) -> Stream:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT file_path FROM waveforms WHERE id = ?",
                (waveform_id,)
            )
    ...
    def cleanup_old_data(self, days_to_keep: int = 30) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Delete old records
            cursor.execute("DELETE FROM analysis_results WHERE created_at < ?", (cutoff_time,))
        ...
        logger.info(f"Cleanup complete: {total_deleted} records and {files_removed} files removed")
