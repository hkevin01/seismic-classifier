...
class SignalProcessor:
    ...
    def apply_bandpass_filter(
        ...
        if isinstance(data, Stream):
            ...
            for trace in filtered_stream:
                trace.filter(
                    "bandpass", freqmin=freqmin, freqmax=freqmax, corners=corners
                )
    ...
    def remove_trend(
        self, data: Union[np.ndarray, Stream], method: str = "linear"
    ) -> Union[np.ndarray, Stream]:
        """
    ...
    def apply_taper(
        ...
        if isinstance(data, Stream):
            ...
            logger.info(
                f"Applied {taper_type} taper ({taper_percentage:.1%}) to stream"
            )
    ...
    def preprocess_waveform(
        ...
        for trace in processed_stream:
            ...
            if apply_taper:
                trace.taper(
                    max_percentage=self.config.processing.taper_percentage, type="hann"
                )
...
def calculate_spectral_features(
    data: np.ndarray, sampling_rate: float, nperseg: Optional[int] = None
) -> Dict[str, float]:
    """
...
def calculate_time_domain_features(data: np.ndarray) -> Dict[str, float]:
    ...
    if features["std"] > 0:
        features["skewness"] = np.mean(
            ((data - features["mean"]) / features["std"]) ** 3
        )
        features["kurtosis"] = np.mean(
            ((data - features["mean"]) / features["std"]) ** 4
        )
    else:
...
def estimate_noise_level(
    data: np.ndarray, method: str = "std", window_fraction: float = 0.1
) -> float:
    """
